...
export interface AlignmentLine {
  ...
  edge?: AlignmentEdge; // Which edge of the moving node is aligned
}
...
export class AuxiliaryLayer {
  ...
  render(
    ...
    if (
      this.hoveredNode &&
      !this.selectedNodes.has(this.hoveredNode) &&
      this.hoveredNode !== this.draggingNode
    ) {
      // 如果节点不可见，hover 时也显示淡黄色虚线
      const color = this.hoveredNode.visible
        ? "#ffff00"
        : "rgba(255, 255, 0, 0.5)";
      this.drawBounds(
        ctx,
    ...
    if (this.draggingNode) {
      ...
      if (this.dragTargetNode) {
        ...
        if (bounds && this.rectIntersects(bounds, dirtyRect)) {
          ...
          ctx.font = "12px Arial";
          ctx.fillText(
            `Drop into: ${this.dragTargetNode.name}`,
            bounds.minX,
            bounds.minY - 5
          );
        }
    ...
    if (this.alignmentLines.length > 0) {
      ...
      for (const line of this.alignmentLines) {
        ...
        if (line.edge) {
          ctx.setLineDash([]);
          const label = line.edge.charAt(0).toUpperCase() + line.edge.slice(1);
          if (line.type === "v") {
    ...
    // 7. Draw Selected Node Name at top-right of the element
    this.drawSelectedNodeName(ctx, viewMatrix, dirtyRect);
  }

  private drawSelectedNodeName(
    ctx: CanvasRenderingContext2D,
    viewMatrix: mat3,
    dirtyRect?: Rect
  ) {
    ...
    const bounds = this.getScreenBounds(selectedNode, viewMatrix);
    if (!bounds) return;

    const name = selectedNode.name || `Node ${selectedNode.id}`;
    const text = name;
    ...
    // Position at top-left corner with 5px offset
    let x = bounds.minX;
    let y = bounds.minY - rectHeight - 5;

    // Ensure it stays within canvas bounds
    if (x < 0) {
      x = bounds.maxX + 5; // Flip to right side if no space on left
    }
    if (y + rectHeight > ctx.canvas.height) {
      y = ctx.canvas.height - rectHeight - 5;
    }
    if (y < 0) y = 5;

    // Simple intersection check for dirty rect
    if (dirtyRect) {
      const intersects = !(
        x + rectWidth < dirtyRect.x ||
        x > dirtyRect.x + dirtyRect.width ||
        y + rectHeight < dirtyRect.y ||
        y > dirtyRect.y + dirtyRect.height
      );
      if (!intersects) {
        ctx.restore();
    ...
    ctx.beginPath();
    this.roundRect(ctx, x, y, rectWidth, rectHeight, 4);
    ctx.fill();
  ...
  private rectIntersects(
    ...
    return !(
      bounds.minX > dirtyRect.x + dirtyRect.width ||
